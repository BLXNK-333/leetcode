...
Return the largest prime number that lies on at least one of the diagonals of nums. In case, no
prime is present on any of the diagonals, return 0.
Note that:
An integer is prime if it is greater than 1 and has no positive integer divisors other than 1
and itself.
...
class Solution:
    ...
    def diagonalPrime(self, nums: List[List[int]]) -> int:
        ...
        for i in range(len(nums)):
            ...
            hashset.add(nums[i][len(nums) - i - 1])

        primes = []
        for num in hashset:
            if self.is_prime(num):
                primes.append(num)
        return 0 if not primes else max(primes)
