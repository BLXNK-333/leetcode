...
class Solution:
    def reorderSpaces(self, text: str) -> str:
        ...
        if not s:
            return text
        text = text.split()
        if len(text) == 1: return text[0] + " " * s

        x, y = divmod(s, len(text) - 1)
        return -1 if not x else f'{" " * x}'.join(text) + " " * y
...
if __name__ == '__main__':
    ...
    assert sol.reorderSpaces("practice") == "practice"
