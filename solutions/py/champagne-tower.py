...
right of those glasses, and so on.  (A glass at the bottom row has its excess champagne fall on
the floor.)
...
(0, 0)). There is one cup of excess liquid. The glass indexed as (1, 0) and the glass indexed as
...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        while len(dp) <= query_row:
            next_row = [0] * (len(dp) + 1)
            for i, glass in enumerate(dp):
                next_glass = max(0, (glass - 1) / 2)
                next_row[i] += next_glass
                next_row[i + 1] += next_glass
            dp = next_row
