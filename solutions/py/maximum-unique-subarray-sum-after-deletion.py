...
class Solution:
    def maxSum(self, nums: List[int]) -> int:
        ...
        x = sum(d for d in hset if d > 0)
        if x:
            return x
        if 0 in hset:
            return 0
        return max(d for d in hset if d < 0)


if __name__ == '__main__':
    sol = Solution()
    print(sol.maxSum(nums=[1, 2, 3, 4, 5]))
    print(sol.maxSum(nums=[1, 1, 0, 1, 1]))
    print(sol.maxSum(nums=[1, 2, -1, -2, 1, 0, -1]))

    print(sol.maxSum(nums=[-10, -20]))
