...
plants in order from right to left, starting from the (n - 1)th plant. They begin watering the
...
and Bob's watering cans respectively, return the number of times they have to refill to water
all the plants.
...
2, so he refills his can then waters it.
...
- Since neither of them have enough water for their current plants, they refill their cans and
...
- Alice's watering can has 10 units of water, whereas Bob's can has 8 units. Since Alice has
more water in her can, she waters this plant.
...
class Solution:
    def minimumRefill(self, plants: List[int], capacityA: int, capacityB: int) -> int:
        ...
        if not len(plants) & 1:
            ...
            if ra == rb:
                result += not (ra // plants[mid])
            else:
                result += not (max(ra, rb) // plants[mid])
        return result


if __name__ == '__main__':
    sol = Solution()
    assert sol.minimumRefill(plants=[2, 2, 3, 3], capacityA=5, capacityB=5) == 1
    assert sol.minimumRefill(plants=[2, 2, 3, 3], capacityA=3, capacityB=4) == 2
    assert sol.minimumRefill(plants=[5], capacityA=10, capacityB=8) == 0
    assert sol.minimumRefill(plants=[1, 2, 4, 4, 5], capacityA=6, capacityB=5) == 2
