...
Output: 8
Explanation: Between each of the following device pairs, there is one beam. In total, there are
8 beams:
 * bank[0][1] -- bank[2][1]
 * bank[0][1] -- bank[2][3]
 * bank[0][2] -- bank[2][1]
...
class Solution:
    def numberOfBeams(self, bank: List[str]) -> int:
        ...
        for row in bank:
            current = row.count('1')
            if current:
                counter_lasers += current * prev
                prev = current
        return counter_lasers
...
if __name__ == '__main__':
    ...
    assert sol.numberOfBeams(["0", "0"]) == 0
