...
class Solution:
    def get_child(self, comb: str):
        child = []
        for i in range(4):
            for j in (1, -1):
                candidate = list(comb)
                candidate[i] = str((int(candidate[i]) + j) % 10)
                child.append("".join(candidate))
        return child
    ...
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        while queue:
            cnt, candidate = queue.popleft()
            if candidate == target:
                return cnt
            for comb in self.get_child(candidate):
                if comb in visited or comb in block:
                    continue
                visited.add(comb)
                queue.append([cnt + 1, comb])
        return -1
