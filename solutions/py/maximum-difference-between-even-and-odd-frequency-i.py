...
class Solution:
    def maxDifference(self, s: str) -> int:
        ...
        odd = [v for v in hm.values() if v % 2]
        even = [v for v in hm.values() if not v % 2]
...
if __name__ == '__main__':
    sol = Solution()
    assert sol.maxDifference(s="aaaaabbc") == 3
    assert sol.maxDifference(s="abcabcab") == 1
    assert sol.maxDifference(s="zgzaaa") == 1
    assert sol.maxDifference(s="tzt") == -1
