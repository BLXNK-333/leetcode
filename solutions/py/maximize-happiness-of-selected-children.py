...
becomes [0,1].
- Pick the child with the happiness value == 1. The happiness value of the remaining child
becomes [0]. Note that the happiness value cannot become less than 0.
...
Output: 1
Explanation: We can pick 2 children in the following way:
- Pick any child with the happiness value == 1. The happiness value of the remaining children
becomes [0,0,0].
- Pick the child with the happiness value == 0. The happiness value of the remaining child
becomes [0,0].
...
- Pick the child with the happiness value == 5. The happiness value of the remaining children
becomes [1,2,3].
...
class Solution:
    def maximumHappinessSum(self, happiness: List[int], k: int) -> int:
        ...
        for child in happiness:
            if k < 1 or (x := child - negative) <= 0:
                break
            counter += x
            negative += 1
            k -= 1
...
if __name__ == "__main__":
    sol = Solution()
    sol.maximumHappinessSum(happiness=[2, 3, 4, 5], k=1)
