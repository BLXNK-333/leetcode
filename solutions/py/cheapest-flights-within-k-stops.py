...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        for frm, to, dist in flights:
            graph[frm].append((to, dist))
        pq = [(0, src, k + 1)]

        while pq:
            dist, node, counter = heapq.heappop(pq)
            if node == dst:
                return dist
            if counter > 0:
                for child, next_dist in graph.get(node, []):
                    heapq.heappush(pq, (dist + next_dist, child, counter - 1))
        return -1


if __name__ == '__main__':
    sol = Solution()
    assert sol.findCheapestPrice(n=4, flights=[[0, 1, 100], [1, 2, 100], [2, 0, 100], [1, 3, 600], [2, 3, 200]], src=0,
                                 dst=3, k=1) == 700

    assert sol.findCheapestPrice(n=5, flights=[[4, 1, 1], [1, 2, 3], [0, 3, 2], [0, 4, 10], [3, 1, 1], [1, 4, 3]],
                                src=2, dst=1, k=1) == -1
