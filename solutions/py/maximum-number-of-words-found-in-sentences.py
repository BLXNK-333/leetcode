...
Thus, the maximum number of words in a single sentence comes from the third sentence, which has
6 words.
...
class Solution:
    def mostWordsFound(self, sentences: List[str]) -> int:
        mx = max(s.count(' ') for s in sentences)
        return 1 if not mx else mx + 1