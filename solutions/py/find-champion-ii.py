...
same node as node an+1, the nodes a1, a2, ..., an are distinct, and there is a
...
class Solution:
    def findChampion(self, n: int, edges: List[List[int]]) -> int:
        diff = ({i[0] for i in edges} | {*range(n)}) - {i[1] for i in edges}
        return diff.pop() if len(diff) == 1 else -1
