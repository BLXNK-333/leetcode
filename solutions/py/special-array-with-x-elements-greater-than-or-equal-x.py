...
class Solution:
    def bi_search(self, value, array):
        L, R = -1, len(array)
        while L + 1 < R:
            M = (L + R) // 2
            if array[M] >= value:
    ...
    def specialArray(self, nums: List[int]) -> int:
        ...
        for val in range(n + 1):
            idx = self.bi_search(val, sorted_nums)
            if n - idx == val:
                return val
        return -1
...
if __name__ == '__main__':
    sol = Solution()
    print(sol.specialArray(nums=[3, 5]))
    print(sol.specialArray(nums=[0, 0]))
    print(sol.specialArray(nums=[0, 4, 3, 0, 4]))
