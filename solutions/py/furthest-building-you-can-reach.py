    ...
    def furthestBuilding(self, heights: List[int], bricks: int, ladders: int) -> int:
        ...
        while i + 1 < len(heights):
            if heights[i] < heights[i + 1]:
                ...
                elif min_heap and min_heap[0] < diff and bricks >= min_heap[0]:
...
if __name__ == '__main__':
    ...
    assert sol.furthestBuilding(heights=[4, 12, 2, 7, 3, 18, 20, 3, 19], bricks=10, ladders=2) == 7
    assert sol.furthestBuilding(heights=[4, 9, 10, 11, 12, 13], bricks=5, ladders=1) == 5
