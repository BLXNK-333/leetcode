...
'a').
Return a string formed by concatenating the mapped characters for all words in
order.
...
8, which maps to 'r'.
The weight of "def" is 14 + 1 + 2 = 17. The result modulo 26 is 17 % 26 = 17,
which maps to 'i'.
The weight of "xyz" is 7 + 7 + 2 = 16. The result modulo 26 is 16 % 26 = 16,
which maps to 'j'.
...
class Solution:
    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        letters_weights = dict(zip(ascii_lowercase, weights))
        reverse_alphabet = ascii_lowercase[::-1]
        final_string = ""

        for word in words:
            letter_position = sum(letters_weights.get(w) for w in word) % 26
            final_string += reverse_alphabet[letter_position]
