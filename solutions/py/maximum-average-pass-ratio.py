...
Output: 0.78333
Explanation: You can assign the two extra students to the first class. The average pass ratio
will be equal to (3/4 + 3/5 + 2/2) / 3 = 0.78333.
...
class Solution:
    def change_classes(self, array: list) -> None:
        for i in range(len(array)):
            pass_, total = array[i]
            ratio = pass_ / total
            k = ((pass_ + 1) / (total + 1)) - ratio
            array[i] = [-k, ratio, pass_ + 1, total + 1]
    ...
    def maxAverageRatio(self, classes: List[List[int]], extraStudents: int) -> float:
        ...
        while extraStudents:
            k, rat, p, t = heapq.heappop(classes)
            new_rat = rat - k
            new_k = ((p + 1) / (t + 1)) - new_rat
            heapq.heappush(classes, [-new_k, new_rat, p + 1, t + 1])
            extraStudents -= 1

        return sum(cl[1] for cl in classes) / len(classes)
