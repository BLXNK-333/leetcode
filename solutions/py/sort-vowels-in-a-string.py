...
Input: s = "lYmpH"
Output: "lYmpH"
Explanation: There are no vowels in s (all characters in s are consonants), so we return
"lYmpH".
...
class Solution:
    def sortVowels(self, s: str) -> str:
        ...
        array, j = iter(sorted(ltr for ltr in s if ltr in vowels)), 0
        return ''.join(next(array) if ltr in vowels else ltr for ltr in s)
