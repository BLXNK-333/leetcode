...
score, the 2nd place athlete has the 2nd highest score, and so on. The placement of each athlete
...
class Solution:
    def findRelativeRanks(self, score: List[int]) -> List[str]:
        n = len(score)
        array = zip(sorted(zip(score, range(n)), reverse=True), range(1, n + 1))
        prize = {1: "Gold Medal", 2: "Silver Medal", 3: "Bronze Medal"}
        result = [""] * n

        for (score, index), place in array:
            if place < 4:
                result[index] = prize[place]
            else:
