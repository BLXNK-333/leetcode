...
You are given 4 strings arriveAlice, leaveAlice, arriveBob, and leaveBob. Alice
will be in the city from the dates arriveAlice to leaveAlice (inclusive), while
Bob will be in the city from the dates arriveBob to leaveBob (inclusive). Each
...
class Solution:
    ...
    def convert_date_to_days(self, date: str):
        start = int(date[:2])
        end = int(date[3:])
        return sum(self.days_in_months[:start - 1]) + end

    def countDaysTogether(self, arriveAlice: str, leaveAlice: str, arriveBob: str,
                          leaveBob: str) -> int:
        a_start = self.convert_date_to_days(arriveAlice)
        a_end = self.convert_date_to_days(leaveAlice)
        b_start = self.convert_date_to_days(arriveBob)
        b_end = self.convert_date_to_days(leaveBob)
