...
class Solution:
    def spiralMatrix(self, m: int, n: int, head: Optional[ListNode]) -> List[List[int]]:
        ...
        while head is not None:
            x = i + di
            y = j + dj
            if 0 <= x < m and 0 <= y < n and mtrx[x][y] == -1:
                mtrx[x][y] = head.val
                head = head.next
                i, j = x, y
            else:
...
if __name__ == '__main__':
    sol = Solution()
    ARRAY = [3, 0, 2, 6, 8, 1, 7, 9, 4, 2, 5, 5, 0]
    HEAD = CUR = ListNode(ARRAY[0])

    for idx in range(1, len(ARRAY)):
        CUR.next = ListNode(ARRAY[idx])
        CUR = CUR.next

    print(*sol.spiralMatrix(3, 5, HEAD), sep="\n")
