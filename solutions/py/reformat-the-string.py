...
class Solution:
    def reformat(self, s: str) -> str:
        ...
        result = list(chain(*zip(*sorted([digits, letters], key=len, reverse=True))))

        ld, ll = len(digits), len(letters)
        if ld != ll:
            result.append(digits[-1] if ld > ll else letters[-1])
        return "".join(result)


if __name__ == '__main__':
    sol = Solution()
    print(sol.reformat(s = "a0b1c2"))
