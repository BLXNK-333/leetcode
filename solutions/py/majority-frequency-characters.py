...
The frequency group for a value k is the set of characters that appear exactly
k times in s.
...
majority frequency group. There is a tie in group size with frequency 1, but we
pick the higher frequency: 2.
...
class Solution:
    def majorityFrequencyGroup(self, s: str) -> str:
        ...
        for k, v in freq_map.items():
            alpha_map[v] = alpha_map.get(v, []) + [k]

        max_item = max(alpha_map.items(), key=lambda item: (len(item[1]), item[0]))
        result = "".join(max_item[1])
...
if __name__ == '__main__':
    sol = Solution()
    assert sol.majorityFrequencyGroup(s="aaabbbccdddde") == "ab"
    assert sol.majorityFrequencyGroup(s="abcd") == "abcd"
    assert sol.majorityFrequencyGroup(s="pfpfgi") == "pf"
    assert sol.majorityFrequencyGroup(s="asrhyrmzhcehcydmrmce") == "rhmc"
