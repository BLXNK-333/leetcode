...
class Solution:
    def intToRoman(self, num: int) -> str:
        ...
        hmap = dict(zip(numbers, "IVXLCDM"))
        result = []

        x, y = num, 1000
        j = len(numbers) - 1
        while y:
            integer, reminder = divmod(x, y)

            if integer:
                if integer in (4, 9):
                    lower = hmap[y]
                    upper = hmap[integer * y + y]
...
if __name__ == '__main__':
    sol = Solution()
    assert sol.intToRoman(num=3749) == "MMMDCCXLIX"
    assert sol.intToRoman(num=58) == "LVIII"
    assert sol.intToRoman(num=1994) == "MCMXCIV"
